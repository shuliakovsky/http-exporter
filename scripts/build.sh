...
GOOS=$GOOS GOARCH=$GOARCH GOARM=$GOARM go build -ldflags "-X main.Version=$(git describe --tags --abbrev=0) -X main.CommitHash=$(git rev-parse --short HEAD)" -o bin/$OUTPUT_NAME cmd/http-exporter/main.go
